...
public:
	...
	/// Print out the tree segments (range covered by each leaf) via the double linked list.
    void print_tree_segments(std::ostream &os) const
    {
		// Start with first leaf:
        const leaf_node *n = headleaf;
		
        // While the leaf is not NULL:
		while(n)
        {
			// Send first and last key entry in this leaf to os:
            os << n->slotkey[0] << " " << n->slotkey[n->slotuse - 1] << std::endl;
